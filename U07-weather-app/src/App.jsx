...
function App() {
  ...
  const [hourURL, sethourURL] = useState(null);
  ...
  const handleLocationClick = async () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        sethourURL(weatherData.forecast.forecastday[0].hour[0].temp_c);
      });
    }
  };
  ...
  const getCurrenthour = () => {
    ...
    //sethourURL(weatherData.forecast.forecastday[0].hour);
    return  hourURL;
  };
  ...
  return (
    <div>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor"  d="M12 2v8m-7.07.93l1.41 1.41M2 18h2m16 0h2m-2.93-7.07l-1.41 1.41M22 22H2M8 6l4-4l4 4m0 12a4 4 0 0 0-8 0"/></svg>
        ...
        <div className="hour">
          <p className="hour"> hour1: {hourURL} </p>
        </div>
    </div>
  );
}
